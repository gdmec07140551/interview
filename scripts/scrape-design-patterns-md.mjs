/**
 * Scrape Feishu design patterns interview docx into content/design-patterns/
 * Source: https://w0hog67yl81.feishu.cn/wiki/CAAaw7kNBi8icxkzcFAcBRVVnje
 */
...
const outDir = path.join(root, 'content', 'design-patterns')
const WIKI_TOKEN = 'CAAaw7kNBi8icxkzcFAcBRVVnje'
const DOC_TOKEN = 'CxBHdHuSyoFt2dxuLogc3NsMnrc'
const SOURCE = `https://w0hog67yl81.feishu.cn/wiki/${WIKI_TOKEN}`
const MODULE_ID = 'design-patterns'
...
async function main() {
  ...
  console.log('open design-patterns wiki doc…')
  ...
  if (!sections.length) throw new Error('no heading3 sections found')
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    const bodyBlocks = blocks.filter((b) => b.data?.type !== 'heading3')
...
`
  ...
  // Keep sidebar order by question number
  docsMeta.sort((a, b) => {
    const na = parseInt(a.slug, 10) || 0
    const nb = parseInt(b.slug, 10) || 0
    return na - nb
  })

  const links = docsMeta.map((d) => `- [${d.title}](/m/${MODULE_ID}/${d.slug})`).join('\n')
  const indexMd = `# 设计模式面试题 · 目录
...
`
  ...
  mod.title = `设计模式（${docsMeta.length}）`
  mod.docs = [
  ...
  await fs.writeFile(
    path.join(root, 'scripts', 'design-patterns-feishu-map.json'),
