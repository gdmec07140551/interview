/**
 * Scrape Feishu frontend security interview docx into content/security/
 * Source: https://w0hog67yl81.feishu.cn/wiki/YQDHw5T6ViPKKwkwL9RcyXuZnSh
 */
...
const outDir = path.join(root, 'content', 'security')
const WIKI_TOKEN = 'YQDHw5T6ViPKKwkwL9RcyXuZnSh'
const DOC_TOKEN = 'JwxDd4xB0or3CJxvUi5cNrdOnuc'
...
async function main() {
  ...
  console.log('open security wiki doc…')
...
`
  ...
  mod.title = `前端安全（${docsMeta.length}）`
  mod.docs = [
  ...
  await fs.writeFile(
    path.join(root, 'scripts', 'security-feishu-map.json'),
