/**
 * Scrape Feishu TypeScript interview docx into per-question pages under content/typescript/
 * Source: https://w0hog67yl81.feishu.cn/wiki/RppqwPYuyiRxzLk2dPFc7oY4nzf
 */
...
const outDir = path.join(root, 'content', 'typescript')
const WIKI_TOKEN = 'RppqwPYuyiRxzLk2dPFc7oY4nzf'
const DOC_TOKEN = 'WMbcdT8TooTe1gxTVpRcFpH0nwe'
const SOURCE = `https://w0hog67yl81.feishu.cn/wiki/${WIKI_TOKEN}`
const MODULE_ID = 'typescript'
...
function appendBlocks(lines, blocks, blockMap, depth = 0) {
  ...
  for (const block of blocks) {
    ...
    if (type === 'code') {
      const lang = (block.data.language || '').toLowerCase() || 'typescript'
...
async function main() {
  ...
  console.log('open TypeScript wiki doc…')
...
`
  ...
  const indexMd = `# TypeScript 面试题 · 目录
...
`
  ...
  mod.title = `TypeScript（${docsMeta.length}）`
  mod.docs = [
  ...
  await fs.writeFile(
    path.join(root, 'scripts', 'typescript-feishu-map.json'),
