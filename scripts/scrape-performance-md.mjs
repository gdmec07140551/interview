/**
 * Scrape Feishu frontend performance interview docx into content/performance/
 * Source: https://w0hog67yl81.feishu.cn/wiki/BVfzwnNBiibevfkIN1Acyaspnkf
 */
...
const outDir = path.join(root, 'content', 'performance')
const WIKI_TOKEN = 'BVfzwnNBiibevfkIN1Acyaspnkf'
const DOC_TOKEN = 'JAmWdzWjAoFQPtxIEIxcE68dnih'
const SOURCE = `https://w0hog67yl81.feishu.cn/wiki/${WIKI_TOKEN}`
const MODULE_ID = 'performance'
...
async function main() {
  ...
  console.log('open performance wiki doc…')
...
`
  ...
  mod.title = `性能优化（${docsMeta.length}）`
  mod.docs = [
  ...
  await fs.writeFile(
    path.join(root, 'scripts', 'performance-feishu-map.json'),
