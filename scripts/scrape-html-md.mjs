/**
 * Scrape Feishu HTML interview docx into per-question pages under content/html/
 * Source: https://my.feishu.cn/docx/GcXLdcs8LoZ1ARxtlVvcPUVUnOb
 */
...
const outDir = path.join(root, 'content', 'html')
const DOC_TOKEN = 'GcXLdcs8LoZ1ARxtlVvcPUVUnOb'
...
function appendBlocks(lines, blocks, blockMap, depth = 0) {
  ...
  for (const block of blocks) {
    ...
    if (type === 'code') {
      const lang = (block.data.language || '').toLowerCase() || 'html'
...
`
  ...
  mod.title = `HTML 面试题库（${docsMeta.length}）`
  mod.docs = [
  ...
  await fs.writeFile(
    path.join(root, 'scripts', 'html-feishu-map.json'),
