/**
 * Scrape Feishu JS handwrite interview docx into content/js-handwrite/
 * Source: https://w0hog67yl81.feishu.cn/wiki/WA8Cw0NJFiEDSWkfEnNciVmrnww
 *
 * Structure: single docx, 5 heading2 categories, 50 Q&As as numbered text
 * ("N. 问题： ...") + answer/code blocks.
 */
...
const outDir = path.join(root, 'content', 'js-handwrite')
const WIKI_TOKEN = 'WA8Cw0NJFiEDSWkfEnNciVmrnww'
const DOC_TOKEN = 'ZKtbdRH9goZZRAx6n87csOk8nOd'
const SOURCE = `https://w0hog67yl81.feishu.cn/wiki/${WIKI_TOKEN}`
const MODULE_ID = 'js-handwrite'
...
function isQuestionTitle(block) {
  ...
  return /^\d+[.、]\s*问题/.test(blockText(block).trim())
}
...
async function main() {
  ...
  console.log('open js-handwrite wiki doc…')
...
`
  ...
  mod.title = `JS 手写题（${docsMeta.length}）`
  mod.docs = [
  ...
  await fs.writeFile(
    path.join(root, 'scripts', 'js-handwrite-feishu-map.json'),
