...
> 来源：[飞书原文](https://w0hog67yl81.feishu.cn/wiki/QApVwZbq0iPctdkBEl3cKaybn3g)  
> 章节：一、渲染流程与性能优化
...
1. 跳过布局和绘制: 当你为一个被提升到独立图层的元素应用 transform (移动、缩放、旋转) 或 opacity (改变透明度) 动画时，浏览器能够识别出这个变化不会影响到其他元素的布局 (Layout)，因此可以完全跳过 Layout 和 Paint这两个最耗时的阶段。
