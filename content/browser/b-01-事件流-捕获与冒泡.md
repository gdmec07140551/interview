...
> 来源：[飞书原文](https://w0hog67yl81.feishu.cn/wiki/KUAIwk1PtiMvmmkoHDecqbCenFg)  
> 章节：二、DOM与事件模型
...
1. 捕获阶段 (Capturing Phase): 浏览器会从最外层的祖先 window 开始，像一个“指挥官”下达命令一样，逐级向下“探查”，经过 document、&lt;html&gt;、&lt;body&gt;、div、p，最终到达你点击的目标 span。这个从外到内的过程就是捕获阶段。
2. 目标阶段 (Target Phase): 事件到达了它的最终目的地——你实际点击的 span 元素。事件在这里被触发。
3. 冒泡阶段 (Bubbling Phase): 事件完成任务后，开始“汇报工作”。它会从 span 开始，逐级向上“冒泡”，经过 p、div，一直到 window。这个从内到外的过程就是冒泡阶段。
